...
<body>
  ...
  <nav class="navbar navbar-expand-xs navbar-dark fixed-top">
    <!-- Button to trigger collapsible menu -->
    ...
    <a class="navbar-brand" href="javascript:void(0);" onclick="editpic()">
      <img src="https://lh5.googleusercontent.com/d/15oBJkXkg-WVElsZb6a-BlRx8CyPP0_Q5" alt="ลงเวลา" id="avatar"
    ...
    <div class="collapse navbar-collapse" id="collapsibleNavbar">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link" href="javascript:void(0);" onclick="checkMap()">
            <i class="fas fa-map-marker-alt"></i> ตรวจสอบพิกัด
          </a>
        </li>
   <li class="nav-item">
          <a class="nav-link" href="javascript:void(0);" onclick="canceltoday()">
            <i class="fas fa-xmark"></i> ยกเลิกการลงเวลาในวันนี้
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="javascript:void(0);" onclick="openWebAdmin()">
            <i class="fas fa-chart-bar"></i> รายงานและการจัดการ
          </a>
        </li>
           <li class="nav-item">
          <a class="nav-link" href="authen.html">
            <i class="fab fa-telegram"></i> เชื่อมต่อ Telegram
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="javascript:void(0);" onclick="openWeb5s()">
            <i class="fas fa-folder"></i> งาน 5ส.
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="javascript:void(0);" onclick="aboutme()">
            <i class="fas fa-user"></i> ข้อมูลส่วนตัว
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="javascript:void(0);" onclick="editpic()">
            <i class="fas fa-image"></i> อัปเดตโปรไฟล์
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="javascript:void(0);" onclick="clearLocal()">
            <i class="fas fa-sync-alt"></i> ออกจากระบบ
          </a>
        </li>
      </ul>
    </div>
    ...
    <div class="login-box">
      ...
      <form>
        <div style="text-align: center;">
          <input type="datetime-local" id="daytime" class="daytime" name="daytime" required="true">
        </div>
        ...
        <!--               <div id="cin" class="invisible"><button id="cin" type="button" class="btn btn-success" onclick="checkin()">ยื่นคำขอลงเวลามา</button></div>
              
              <div id="cout" class="invisible"><button id="cout" type="button" class="btn btn-danger" onclick="checkout()">ยื่นคำขอลงเวลากลับ</button></div>    -->
...
<div class="modal" id="loadingModal" data-backdrop="static" data-keyboard="false">
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <div class="signup-link"><a
              href="https://social-plugins.line.me/lineit/share?url=https://liff.line.me/1654797991-BXng4Kgv">ลงทะเบียน</a>
          </div>
          <button type="button" class="btn btn-success" onclick="getLocationcheck()">ทดสอบ</button>
          <button type="button" class="btn btn-info"
            onclick="window.location.href='https://liff.line.me/1654797991-BXng4Kgv';">ลงทะเบียน</button>
          <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="fa-solid fa-xmark">ปิด</button>
        </div>
      </div>
    </div>
  ...
  <script>
    window.onload = function checks() {
      let today = new Date();
      today.toLocaleString('th-TH', { timeZone: 'Asia/Bangkok' })
      let hm = today.toLocaleTimeString('th-TH');
      ...
      if (day === 0 || day === 6) {
        // กำหนดค่าให้กับอิลิเมนต์ที่มี id เป็น "typea"
        document.getElementById("typea").value = "วันหยุด";
      }
      ...
      let yesterdayFormatted = yesterday.toISOString().split(".")[0].slice(0, -8);
      // Set the minimum value for the "daytime" input element  
      document.getElementById("daytime").value = dateTime + "08:30";
      document.getElementById("daytime").min = yesterdayFormatted + "08:30";
      document.getElementById("daytime").max = dateTime + "16:30";
    ...
    if (avatarSrc) {
      ...
      document.getElementById("avatar").src = "default-avatar.png"; // ใส่รูปภาพที่เป็นค่าเริ่มต้น
    }
